...
export function AIChat({
  ...
  const [kbStoreMode, setKbStoreMode] = useState<KnowledgeStoreMode>(() =>
    loadKbStoreMode(),
  );
  ...
  const send = async (overrideText?: string) => {
    ...
    if (viewCmd) {
      ...
      const target = kEntries.find(
        (e) =>
          e.name.trim().toLowerCase() === kbSaveCmd.title.trim().toLowerCase(),
      );
      if (target) {
