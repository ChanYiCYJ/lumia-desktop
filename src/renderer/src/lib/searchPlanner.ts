...
import { searchBackend, searchAI, searchFast, detectQueryLang, detectQueryType } from './search'
...
function diversifyByDomain(results: SearchResult[], max: number, perDomain = 2): SearchResult[] {
  ...
  for (const r of results) {
    ...
    const cap = host.includes('bilibili.com') || host.includes('bgm.tv') ? 4 : perDomain
...
const SPLIT_RE = /(?:和|与|以及|还有|还有啥|另外|此外|同时|顺便|对比|比较|区别|vs\.?|VS|，|；|;)/
 ...
 */
export function enrichKeywords(query: string, lang: string, type: QueryType): string {
  const q = (query || '').trim()
  if (!q) return q
  const parts = [q]
...
function rankResult(r: SearchResult, qTokens: string[]): number {
  ...
  if (
    /(^|\.)(bing|duckduckgo|baidu|google|qwant|mojeek)\.(com|org|net|cn|co\.jp|io)$/.test(host) &&
  ...
  if (title.includes(currentYear) || (r.description || '').includes(currentYear)) {
    score += 2
  }
  // 模板/无意义标题
...
export function rankAndConsolidate(
  ...
  for (const r of urlDeduped) {
    ...
    if (
      seenTitle.some((s) => Math.abs(s.length - norm.length) <= 1 && overlapScore(s, norm) >= 0.85)
    ) {
      continue
    }
    seenTitle.push(norm)
  ...
  return titleDeduped
    ...
    .sort((a, b) => b.s - a.s || (b.r.title || '').length - (a.r.title || '').length)
    .slice(0, limit)
    .map((x) => x.r)
}
...
export function applyFeedbackToSearch(queryType: QueryType, lang: string, rating: 1 | -1): void {
  ...
  // 清理过期 & 限数
  const valid = pat.filter((p) => now - p.updatedAt < PATTERN_TTL_MS).slice(-PATTERN_MAX)
  savePatterns(valid)
}
...
export function getPatternScore(type: QueryType, lang: string): number {
  ...
  const pat = loadPatterns()
  const now = Date.now()
  const hit = pat.find((p) => p.queryType === key && now - p.updatedAt < PATTERN_TTL_MS)
...
export interface SearchProgress {
  stage: 'thinking' | 'planning' | 'searching' | 'correcting' | 'merging' | 'done'
  current?: number
  total?: number
  subQuery?: string
  message?: string
...
export async function searchSegmented(
  ...
  const merged = diversifyByDomain(filterRelevant(dedupeByUrl(all), q, type), limit, 2)
