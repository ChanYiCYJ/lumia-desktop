...
/** 带超时的 fetch */
async function fetchImageJson(url: string, timeoutMs = 12000): Promise<unknown | null> {
  try {
...
async function searchViaProxy(query: string, type: string, limit: number): Promise<ImageResult[]> {
  // 桌面端：主进程本地图片引擎
  if (isNative()) {
    const j = (await image({
      keyword: query,
...
async function searchWikimediaDirect(query: string, limit: number): Promise<ImageResult[]> {
  const j = await fetchImageJson(
  ...
  const pages = (j as { query?: { pages?: Record<string, unknown> } } | null)?.query?.pages
  if (!pages) return []
 ...
 */
export async function animeSearchKeyword(query: string, cfg?: AICfgLike | null): Promise<string> {
...
export async function imageSearch(
  ...
  const type = category === 'anime' ? 'anime' : category === 'product' ? 'product' : 'photo'
...
export function imageUrls(results: ImageResult[], max = 3): string[] {
  ...
  for (const r of results) {
    ...
    if (/^https?:\/\//i.test(u) && !u.includes('data:') && !u.endsWith('.svg')) {
