...
function playEmotionMotion(emotion: Emotion): boolean {
  ...
  const candidates = (EMOTION_MOTIONS[emotion] || []).filter((n) => names.has(n))
  ...
  if (candidates.length > 1 && emotion !== 'neutral') {
    ...
    if (other === a) other = candidates[(candidates.indexOf(a) + 1) % candidates.length]
    b = other
  }
  try {
...
export function setEmotion(emotion: Emotion): void {
  ...
  const mouthW = lipEnd && Date.now() - lipEnd < 300 ? Math.min(1, (Date.now() - lipEnd) / 300) : 1
...
export async function loadModel(name: string): Promise<void> {
  ...
  try {
    ...
    // 纹理 404 探测 → 透明 PNG 兜底（防破图）
    const checks = await Promise.all((settings.textures || []).map((u) => canLoadImage(u)))
    let firstValid = TRANSPARENT_PNG_DATA_URL
    const idx = checks.findIndex(Boolean)
    if (idx >= 0) firstValid = settings.textures[idx]
    settings.textures = (settings.textures || []).map((u, i) => (checks[i] ? u : firstValid))
...
function handleTap(clientX: number, clientY: number): void {
  ...
  const region = resolveHitRegion(px, py, model.x, model.y, modelBaseW * s, modelBaseH * s)
  if (!region) return
...
function paramExists(core: any, id: string): boolean {
  try {
    return typeof core.getParamIndex === 'function' && core.getParamIndex(id) >= 0
  } catch {
    return false
  }
}
...
export function applyActionCommands(text: string): void {
  ...
  if (!cmds.params.length && !cmds.motion && !cmds.expression && !cmds.look) return
 ...
 */
const MOUTH_PARAMS: readonly string[] = ['ParamMouthOpenY', 'PARAM_MOUTH_OPEN_Y']
...
function buildLipSyncStep(
  ...
  return () => {
    ...
    if (freqA) {
      ...
      const bands = analyzeFrequencyBands(freqData, audioCtx?.sampleRate || 44100)
...
function fadeMouthToZero(): void {
  ...
  const step = (now: number) => {
    ...
    setMouthFormParams(core, fromW * (1 - e), fromR * (1 - e), fromSc + (1 - fromSc) * e)
 ...
 */
export function speakAudio(url: string, opts?: { volume?: number; onEnd?: () => void }): boolean {
  stopSpeaking()
