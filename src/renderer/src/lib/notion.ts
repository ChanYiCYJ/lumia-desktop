...
export function saveNotionCfg(cfg: NotionCfg): void {
  localStorage.setItem(
    KEY,
    JSON.stringify({ token: (cfg.token || "").trim() }),
  );
}
...
export function saveNotionSel(sel: NotionSel): void {
  const unique = [
    ...new Set(
      (sel.pageIds || []).filter((x: unknown) => typeof x === "string"),
    ),
  ];
  localStorage.setItem(
    SEL_KEY,
    JSON.stringify({ pageIds: unique.slice(0, 20) }),
  );
}
...
export function parseSearchResponse(data: {
  ...
  for (const r of data.results || []) {
    ...
    const dbTitle = Array.isArray(item.title)
      ? (item.title as { plain_text?: string }[])
          .map((t) => t.plain_text || "")
          .join("")
      : "";
...
export function isPersonalKnowledgeQuery(userMsg: string): boolean {
  ...
  if (/(我的|我创建的)/.test(t) && /(数据库|清单|列表|看板|页面)/.test(t))
    return true;
  return false;
}
...
export function buildNotionContext(
  ...
  for (const r of results.slice(0, 6)) {
    ...
    const edited = r.lastEdited
      ? `（更新于 ${r.lastEdited.slice(0, 10)}）`
      : "";
    lines.push(
...
export async function testNotionConnection(
  ...
  if (!token.trim())
    return { ok: false, message: "请先填写 Notion Integration Token" };
...
export function parseDatabaseSchema(data: {
  ...
  const titleArr = Array.isArray(data.title)
    ? (data.title as { plain_text?: string }[])
        .map((t) => t.plain_text || "")
        .join("")
    : "";
  ...
  for (const [name, raw] of Object.entries(data.properties || {})) {
    ...
    const type = p.type || "";
    const optionList =
      p.select?.options || p.status?.options || p.multi_select?.options || [];
    properties[name] = {
      id: p.id || "",
      name,
      type,
...
export function parseDatabaseRows(data: {
  results?: unknown[];
}): NotionDbRow[] {
...
export async function queryNotionDatabase(
  ...
  try {
    ...
    const data = await notionRequest<{ results?: unknown[] }>(
      `databases/${databaseId}/query`,
      { method: "POST", body, token, fetchImpl },
    );
...
export async function findNotionPageByTitle(
  ...
  try {
    ...
    const hit = rows.find(
      (r) => r.type === "page" && r.title.trim().toLowerCase() === t,
    );
