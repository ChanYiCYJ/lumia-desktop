...
const VALID_PROVIDERS: SearchApiProvider[] = ['auto', 'tavily', 'brave', 'searxng']
...
export function hasSearchApi(cfg: SearchApiCfg | null | undefined): boolean {
  ...
  if (cfg.provider === 'searxng') return /^https?:\/\//i.test(cfg.instance?.trim() || '')
  return false
}
...
function describeError(status: number, detail: string): string {
  if (status === 401 || status === 403) return '认证失败：API Key 无效或没有权限（401/403）'
...
async function testViaProxy(
  ...
  try {
    ...
    if (res.ok) {
      ...
      return {
        ok,
        message: ok ? '连接成功（经代理）' : '代理未返回结果，请检查配置（Key/实例地址）',
        latencyMs,
        provider: cfg.provider
      }
    }
