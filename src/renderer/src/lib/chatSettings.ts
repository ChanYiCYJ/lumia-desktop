...
export function loadSearchSpeed(): ChatSearchSpeed {
  ...
  return SEARCH_SPEEDS.includes(v as ChatSearchSpeed) ? (v as ChatSearchSpeed) : 'standard'
}
...
export function loadSearchDepth(): ChatSearchDepth {
  ...
  return SEARCH_DEPTHS.includes(v as ChatSearchDepth) ? (v as ChatSearchDepth) : 'auto'
}
...
export function buildTtsAudioUrl(template: string, text: string): string | null {
...
export function resolveTtsAudioUrl(
  ...
  const template = source === 'backend' ? BACKEND_TTS_TEMPLATE : thirdPartyUrl || ''
 ...
 */
export function compressMemory(oldMemory: string, question: string, answer: string): string {
  ...
  const deduped = lines.filter((l) => !l.startsWith('- 用户问：') || !l.slice(0, 14).includes(key))
...
export function mergeEffCfg(
  ...
  const selected = activePromptIdx != null ? prompts[activePromptIdx] : undefined
  return {
    ...config,
    endpoint: localCfg.endpoint || config.endpoint,
    apiKey: localCfg.apiKey || config.apiKey,
    model: localCfg.model || config.model,
    systemPrompt: localCfg.prompt || selected?.systemPrompt || config.systemPrompt
  }
}
