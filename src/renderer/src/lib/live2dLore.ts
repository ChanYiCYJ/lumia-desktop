...
export type Live2dLoreDraft = Pick<
  Live2dLore,
  'world' | 'personality' | 'tone' | 'background' | 'likes' | 'relations' | 'notes'
>
...
export function loadAllLore(): Live2dLore[] {
  ...
  try {
    // 不能依赖 Object.keys(localStorage) 前缀扫描——部分环境数据键不可枚举会漏；
    // 统一用 length+key(i) 遍历（同 backupSync/secureStorage）
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i)
      if (!k || !k.startsWith(L2D_LORE_PREFIX)) continue
...
export function loreSearchQuery(name: string, model?: string): string {
  ...
  return known ? `${n} BanG Dream! 角色 设定 性格 背景 世界观` : `${n} Live2D 角色 设定 性格 世界观`
}
