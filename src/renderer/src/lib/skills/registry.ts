...
import { computerUseSection } from './computerUseSkill'
...
export function assembleSystem(ctx: SkillContext): string {
  ...
  // Computer Use（Playwright 浏览器工具存在时）：网页操作指南，紧随本机/MCP 工具段
  const cuSection = computerUseSection(ctx)
  if (cuSection) sections.push(cuSection)
