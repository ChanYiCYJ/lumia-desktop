...
describe('searchPlanner · splitSubQueries 查询分段', () => {
  it('中文「和」拆分', () => {
    const r = splitSubQueries('React 和 Vue 对比', 'zh', generalType(), 'standard')
  ...
  it('中文顿号不拆分（保留整句，避免过度切分噪音）', () => {
    const r = splitSubQueries('苹果、华为、小米', 'zh', generalType(), 'standard')
    expect(r).toEqual(['苹果、华为、小米'])
  })

  it('「还有」拆分', () => {
    const r = splitSubQueries('Python 还有 Rust 区别', 'zh', generalType(), 'fast')
    expect(r.length).toBe(2)
  ...
  it('单主题不拆分', () => {
    const r = splitSubQueries('今天天气怎么样', 'zh', generalType(), 'standard')
    expect(r).toEqual(['今天天气怎么样'])
  })

  it('anime 类型分离主标题和季节后缀', () => {
    const r = splitSubQueries('夏日重现 2026年夏アニメ 推荐', 'ja', animeType(), 'standard')
    expect(r.length).toBe(2)
