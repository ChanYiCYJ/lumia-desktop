...
const sampleBuildData: BuildData = {
  ...
  expressions: [{ bundleName: 'live2d/chara/001_casual', fileName: 'exp01.exp.json' }]
}

describe('live2d · 反代 URL 构造', () => {
  it('model 去掉 .bytes 后缀', () => {
    expect(
      assetUrl({ bundleName: 'live2d/chara/001_casual', fileName: 'model.moc.bytes' }, 'model')
  ...
  it('expression 保留扩展名', () => {
    expect(
      assetUrl({ bundleName: 'live2d/chara/001_casual', fileName: 'exp.exp.json' }, 'expression')
...
describe('live2d · buildLive2dSettings', () => {
  ...
  it('motions 以文件名（去扩展）为 key', () => {
    ...
    expect(s.motions.tap_body?.[0]?.file).toContain('001_general_rip/tap_body.mtn')
  })
...
describe('live2d · detectEmotion 本地规则', () => {
  ...
  it('思考', () => expect(detectEmotion('让我想想 大概是这样')).toBe('thinking'))
  it('普通内容 → neutral', () => expect(detectEmotion('今天天气不错')).toBe('neutral'))
})

describe('live2d · detectReplyEmotion（AI 角色回复情绪，括号表演提示优先）', () => {
  it('括号动作描写优先：吓了一跳 → 惊讶', () => {
    expect(detectReplyEmotion('（突然被吓了一跳，眨眨眼）真的吗？')).toBe('surprised')
  })
  it('括号表演提示优先：声音放软 → 难过（正文开心不误判）', () => {
    expect(detectReplyEmotion('（停了停，声音放软了些）不用勉强自己开心起来')).toBe('sad')
  })
  it('括号：别过脸去 → 害羞', () => {
    expect(detectReplyEmotion('（说完别过脸去，声音小了一点）……开玩笑的啦')).toBe('shy')
  })
  it('无括号时回退整段检测', () => {
  ...
  it('多括号并存且平手：笑出声(happy) 优先于 语气放软(sad)', () => {
    expect(
      detectReplyEmotion('（看你手舞足蹈的样子，忍不住笑出声）（歪了歪头，语气放软）今天真开心！')
    ).toBe('happy')
  })
})
...
describe('live2d · AI 动作指令（参数级精细控制）', () => {
  it('解析 [PARAM:参数:数值] 指令', () => {
    const cmds = parseActionCommands('哈哈[PARAM:ParamMouthOpenY:0.8][PARAM:ParamEyeLOpen:0.5]')
  ...
  it('解析 [MOTION:动作] 与 [EXPRESSION:表情预设]', () => {
    const cmds = parseActionCommands('[MOTION:smile01][EXPRESSION:niyaniya01]正文')
  ...
  it('负值/小数/越界 clamp', () => {
    const cmds = parseActionCommands('[PARAM:ParamBrowLY:-0.5][PARAM:ParamMouthOpenY:2.5]')
    expect(cmds.params.find((p) => p.id === 'ParamBrowLY')?.value).toBe(-0.5)
    // 2.5 越界 → clamp 到 1
    expect(cmds.params.find((p) => p.id === 'ParamMouthOpenY')?.value).toBe(1)
  })
  it('同名参数多次出现保留最后一个', () => {
    const cmds = parseActionCommands('[PARAM:ParamMouthOpenY:0.2][PARAM:ParamMouthOpenY:0.9]')
  ...
  it('剥离动作指令保留正文', () => {
    ...
    expect(stripActionCommands('正文[参数:ParamAngleX:0.4]结尾')).toBe('正文结尾')
    expect(stripActionCommands('无指令正文')).toBe('无指令正文')
  })
  it('与表情标签共存：动作指令解析不受 [表情:] 影响', () => {
    const cmds = parseActionCommands('哈哈[PARAM:ParamMouthOpenY:0.6][表情:开心]')
    expect(cmds.params).toEqual([{ id: 'ParamMouthOpenY', value: 0.6 }])
    expect(cmds.motion).toBeUndefined()
    // 表情标签由 stripEmotionTag 剥离，动作指令由 stripActionCommands 剥离，互不干扰
    expect(stripActionCommands('哈哈[PARAM:ParamMouthOpenY:0.6][表情:开心]')).toBe(
      '哈哈[表情:开心]'
    )
...
describe('live2d · 本地选择 + 站点配置', () => {
  ...
  it('resolveLive2dConfig：enabled 解析 + 默认模型回退', () => {
    ...
    expect(resolveLive2dConfig({ live2d_enable: '0', live2d_model: '011_casual' })).toEqual({
      enabled: false,
      model: '011_casual'
    })
    expect(resolveLive2dConfig({ live2d_enable: '0', live2d_model: '  ' })).toEqual({
      enabled: false,
      model: DEFAULT_LIVE2D_MODEL
    })
  })
...
describe('live2d · 第三方模型导入', () => {
  it('isThirdPartyModelInput 识别网址/路径/.json，模型名不算', () => {
    expect(isThirdPartyModelInput('https://example.com/model/model.json')).toBe(true)
  ...
  it('live2dProxyUrl 构造通用代理 URL', () => {
    expect(live2dProxyUrl('https://example.com/model/model.json')).toBe(
      'lumia-live2d://proxy?url=' + encodeURIComponent('https://example.com/model/model.json')
    )
  })
...
describe('live2d · 角色分组 + 自定义模型导入', () => {
  ...
  it('add/load/remove 自定义模型（去重 + 持久化）', () => {
    ...
    expect(loadCustomModels()).toEqual([{ model: '026_casual', name: '自定义角色' }])
    // 空值不写入
    addCustomModel('   ')
...
describe('live2d · bandEnergiesToMouthParams（频带→嘴型映射）', () => {
  ...
  it('高频主导 → 嘴型偏宽', () => {
    const mp = bandEnergiesToMouthParams({ low: 0.1, mid: 0.2, high: 0.7 }, 0.3)
    expect(mp.width).toBeGreaterThan(0)
  })

  it('低频主导 → 嘴型偏圆', () => {
    const mp = bandEnergiesToMouthParams({ low: 0.7, mid: 0.2, high: 0.1 }, 0.3)
    expect(mp.round).toBeGreaterThan(0)
  })

  it('RMS 大 → 缩放 > 1', () => {
    const mp = bandEnergiesToMouthParams({ low: 0.3, mid: 0.3, high: 0.3 }, 0.5)
    expect(mp.scale).toBeGreaterThan(1)
  })

  it('值在合理范围内', () => {
    const mp = bandEnergiesToMouthParams({ low: 0.5, mid: 0.5, high: 0.5 }, 0.3)
...
describe('live2d · Cubism3/4（.model3.json）加载链路', () => {
  ...
  it('buildLive2dSettingsFromModel3 解析 FileReferences + HitAreas（绝对代理 URL）', async () => {
    ...
    expect(s.model).toContain(encodeURIComponent('https://raw.example.com/models/Haru/Haru.moc3'))
  ...
  it('buildLive2dSettingsFromModel3 缺 Moc 抛错', async () => {
    ...
    await expect(buildLive2dSettingsFromModel3('https://x.com/m/M.model3.json')).rejects.toThrow()
  })
  ...
  it('fetchThirdPartyModelSafe 按扩展名分流（model3 vs model.json）', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (u: RequestInfo) => {
        const ustr = String(u)
        calls.push(ustr)
        if (ustr.includes('M.model3.json')) return { ok: true, json: async () => m3 }
        return { ok: true, json: async () => m2 }
      })
    )
    const s3 = await fetchThirdPartyModelSafe('https://x.com/models/M/M.model3.json')
    expect(s3.model).toContain('a.moc3')
    const s2 = await fetchThirdPartyModelSafe('https://x.com/models/S/S.model.json')
