...
export function detectProvider(endpoint: string, model: string): AiProvider {
  ...
  if (e.includes('moonshot') || m.includes('moonshot') || e.includes('kimi') || m.includes('kimi'))
    return 'kimi'
  if (e.includes('openai') || e.includes('openrouter') || m.includes('gpt')) return 'openai'
  if (e.includes('11434') || m.includes('qwen') || m.includes('ollama')) return 'ollama'
  if (e.includes('1234') || m.includes('lmstudio') || m.includes('local-model')) return 'lmstudio'
  return 'other'
}
...
export function resolveMaxTokens(model: string, fallback = 4096): number {
  ...
  if (m.includes('moonshot') || m.includes('kimi') || m.includes('k2')) return 8192
  return fallback
}
...
export function extractMessage(msg: unknown): {
  ...
  return {
    ...
    reasoning: typeof m.reasoning_content === 'string' ? (m.reasoning_content as string) : ''
  }
}
